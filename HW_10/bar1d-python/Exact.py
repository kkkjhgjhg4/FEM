...
def ExactSolution_HeatConduction(ax1, ax2):
    """
    Plots the exact temperature and derivative of temperature of a elastic bar under
    stable heat source in axes ax1 and ax2, respectively.

    Args:
        ax1 : axis to draw temperature distribution
        ax2 : axis to draw derivative distribution
    """
    # bar parameters
    length = 20.0

    # region of the bar
    x = np.arange(0, length, length / 100)
    # exact temperature field
    T = -10 * np.power(x, 2) + 400 * x

    # plot temperature
    ax1.plot(x, T, '--r', label='Exact')

    # exact temperature derivative for x
    dT = -20 * x + 400

    # plot temperature derivative
    ax2.plot(x, dT, '--r', label='Exact')
